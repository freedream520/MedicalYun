...
CDirectoryWatch::CDirectoryWatch() : m_hFile(INVALID_HANDLE_VALUE), m_pThread(NULL)
{
	...
	QString str = _datachange->readINI("");   //读取.ini文件，获取监测文件路径
	str.replace('/','\\');
	m_pThread->m_bAutoDelete = FALSE;
	m_pThread->ResumeThread();
	_tchar = (wchar_t*) str.utf16();
	//
	StringCchPrintf(m_szDirectory, _countof(m_szDirectory), TEXT("%s"), _tchar);
	m_hFile = CreateFile(_tchar, FILE_LIST_DIRECTORY, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
		NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OVERLAPPED, NULL);
	if(INVALID_HANDLE_VALUE == m_hFile)
	{
		TRACE("Error Code : %d\n", GetLastError());
	}
	//_datachange->writeINI(QString::fromUtf16(_tchar) + "www");
}
